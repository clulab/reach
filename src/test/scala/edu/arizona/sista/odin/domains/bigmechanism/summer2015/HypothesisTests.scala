...
class HypothesisTests extends FlatSpec with Matchers{
    ...
    sen1 should "contain a mention with a hyphotesis modification" in {
    ...
    sen2 should "contain a mention with a hyphotesis modification" in {
    ...
    sen4 should "contain a mention with a hyphotesis modification" in {
      val mentions = parseSentence(sen3).filter(_ matches "Event")
