...
class TestFeedback2 extends FlatSpec with Matchers {
  ...
  s2 should "contain a regulation with NRG as controller" in {
    // TODO: capture controllers when they appear in "stimulated with" constructs - DANE
  }
  ...
  s3 should "NOT contain Activation(Gab1, EGF)" in {
    // TODO: disable activations when the Controlled dep path goes through the trigger of another event (e.g., "activation")
  }

  s3 should "contain PosReg(Gab1, Activation(EGF, PI-3))" in {
    // TODO: capture the positive regulation with Gab1 as Controller and the activation as Controlled
    // this is the complement of the above test
  }
  ...
  s4 should "contain two activations with p53 as Controlled" in {
    ...
    var mentions = getBioMentions(s4)
  ...
  s5 should "contain an activation with a Phosphorylation event serving as Controller" in {
    ...
    //   then convert them to modified entities, e.g., ErbB3_p here
  }
  ...
  s6 should "contain an activation with a PosReg(Phosphorylation) event serving as Controller" in {
    ...
    //   then convert them to modified entities, e.g., Ras_p
    // TOOD: this also needs a new activation pattern: "A results in B activation", "A results in activation of B"
  }
  ...
  s7 should "contain 1 or more translocation events" in {
    // TODO: we should pick at least 1 translocation event here
  }
  ...
  s12 should "contain at least 1 phosphorylation" in {
    // TODO: we miss both phosphos; we should get at least the 1st, which doesn't need coref
  }
  ...
  s14 should "contain 1 activation of MKP3" in {
    // TODO: need new activation pattern: "A results in up-regulation of B"
  }
  ...
  s16 should "contain 1 activation pattern" in {
    // TODO: needs above activation pattern: "A results in up-regulation of B"
  }
  ...
  s17 should "contain phosphorylation at 2 sites" in {
    // TODO: we get the phospho but miss the sites - GUS
  }
  ...
  s18 should "contain 1 reg and 1 phospho at site 853" in {
    // TODO: we get both reg and phospho but miss the site - GUS
  }
}
