...
abstract class RuleBasedContextEngine extends ContextEngine {
  ...
  var defaultContexts:Option[Map[ContextClass.Value, String]] = None
  ...
  def infer(mentions: Seq[BioMention]){
    ...
    val contextMentions = mentions filter ContextClass.isContextMention map (_.asInstanceOf[BioTextBoundMention])
    ...
    val contextCounts:Map[(ContextClass.Value, String), Int] = contextMentions map ContextEngine.getContextKey groupBy identity mapValues (_.size)
    // Then gorup them by class
    val defaultContexts:Map[ContextClass.Value, String] = contextCounts.toSeq.groupBy(_._1._1)
