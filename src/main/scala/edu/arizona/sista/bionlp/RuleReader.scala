...
object RuleReader {
  ...
  case class Rules(entities: String, modifications: String, events: String, coref: String, context: String)
  ...
  val contextDir = s"$resourcesDir/context"
  // For shell
  ...
  def readContextRules(): String = {
    val files = Seq(
      s"$contextDir/context_relations.yml"
    )
  ...
  def reloadRules(): Rules  = {
    ...
    val contextPath = s"$resourcesPath/context"
    ...
    val contextDir = new File(".", contextPath)
    ...
    println(s"\tcontext\t\t=> ${contextDir.getCanonicalPath}")
    ...
    val contextRules = readRuleFilesFromDir(contextDir)
    Rules(entityRules, modificationRules, eventRules, corefRules, contextRules)
  }
  ...
  val POS_NOUNS = "acceler|activ|augment|cataly|caus|driv|elev|elicit|enhanc|increas|induc|induct|initi|produc|promot|promot|rais|reactiv|re-express|releas|stimul|trigger|up-regul|upregul"
  ...
  val NEG_REG_TRIGGERS = "abolish|abrog|absenc|antagon|arrest|attenu|block|blunt|deactiv|decreas|defect|defici|degrad|delay|deplet|deregul|diminish|disengag|disrupt|down|down-reg|downreg|drop|dysregul|elimin|impair|imped|inactiv|inhibit|interf|knock\\-?down|lack|limit|loss|lost|lower|negat|neutral|nullifi|oppos|overc|perturb|prevent|reduc|reliev|remov|repress|resist|restrict|revers|shutdown|slow|starv|suppress|supress|uncoupl"

  // These are a bit stricter than the POS_REG and NEG_REG because the context is more ambiguous for activations
  val POS_ACT_TRIGGERS = "acceler|activat|aid|allow|augment|direct|elev|elicit|enabl|enhanc|increas|induc|initi|modul|necess|overexpress|potenti|produc|prolong|promot|rais|reactivat|rescu|respons|restor|re-express|retent|sequest|signal|stimul|support|synerg|synthes|trigger|up-regul|upregul"
  val NEG_ACT_TRIGGERS = "attenu|block|deactiv|decreas|degrad|diminish|disrupt|impair|imped|inhibit|knock\\-?down|limit|lower|negat|reduc|reliev|repress|restrict|revers|slow|starv|suppress|supress"
