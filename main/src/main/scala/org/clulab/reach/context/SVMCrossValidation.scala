...
object SVMCrossValidation extends App {
  ...
  val pathDirCV = config.getString("svmContext.annotatedPapersDirForCrossVal")
  val outPaperDirPath = config.getString("svmContext.contextOutputDir").concat(s"${pathDirCV}")
