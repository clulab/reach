...
case class LinearSVMContextClassifier(classifier: Option[LinearSVMClassifier[Int,String]] = None, pathToClassifier:Option[String] = None) extends ContextClassifier {
  ...
  def loadFrom(file:File): LinearSVMContextClassifier = {
...
//    val rvfDatumPath = config.getString(("polarityContext.attemptDir")).concat("/RVFDatasetToFile.txt")
//    val os = new ObjectOutputStream(new FileOutputStream(rvfDatumPath))
