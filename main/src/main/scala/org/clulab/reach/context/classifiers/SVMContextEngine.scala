  ...
  override def assign(mentions: Seq[BioMention]): Seq[BioMention] = {
    ...
    paperMentions match {
      ...
      case Some(ctxMentions) =>
        ...
        val inconsistentEventSpans = collection.mutable.HashMap[String,Seq[String]]()
          ...
          case None =>
            {
              ...
              for(p <- pairs) {
                ...
                if(inconsistentAnnotations.size>0){
                  inconsistentEventSpans++=Map(eventID -> inconsistentAnnotations.toSeq)
                }
        ...
        println("The following map of event span pairs were found. Both spans in the pair are events generated by Reach 2019, but the key of the map is the event span generated by Reach on the fly, and the value is the event span as read from the annotations dataset")
        println(inconsistentEventSpans)
