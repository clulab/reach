...
class TestDocumentSerializer extends FlatSpec with Matchers {
  ...
  val ser = new ReachDocumentSerializer
...
"""
  ...
  "DocumentSerializer" should "save/load very small document correctly, keeping text" in {
    val doc1 = ReachDocument(proc.annotate("Z", true))     // explicit keep text
  ...
  "DocumentSerializer" should "save/load small document correctly, with defaults" in {
    val doc1 = ReachDocument(proc.annotate(text))
    // println("Created a document with " + doc1.sentences.size + " sentences.")
  ...
  "DocumentSerializer" should "save/load small document correctly, keeping text" in {
    val doc1 = ReachDocument(proc.annotate(text, true))    // explicit keep text
    // println("Created a document with " + doc1.sentences.size + " sentences.")
    val out1 = ser.save(doc1, keepText=true)
    // println(out1)                          // DEBUGGING
  ...
  "DocumentSerializer" should "save/load large document correctly, with defaults" in {
    val doc1 = ReachDocument(proc.annotate(bigText))
    // println("Created a document with " + doc1.sentences.size + " sentences.")
    val out1 = ser.save(doc1)
    // println(out1)                          // DEBUGGING
  ...
  "DocumentSerializer" should "save/load larger document correctly, keeping text" in {
    val doc1 = ReachDocument(proc.annotate(bigText, true))    // explicit keep text
    // println("Created a document with " + doc1.sentences.size + " sentences.")
    val out1 = ser.save(doc1, keepText=true)
    // println(out1)                         // DEBUGGING
  ...
  "DocumentSerializer" should "save and section names when provided" in {
    ...
    val doc0 = proc.annotate(bigText, true) // explicit keep text
    val arrayOfSectionsOpt = {
      val arrayOfSectionsOpt = doc0.sentences.map(_ => None: Option[Array[String]])

      // Add a section name hierarchy to the first and last sentences of this document
      arrayOfSectionsOpt(0) = Some(firstSection)
      arrayOfSectionsOpt(arrayOfSectionsOpt.length - 1) = Some(lastSections)
      arrayOfSectionsOpt
    }
    val doc1 = ReachDocument(doc0, arrayOfSectionsOpt)
    val out1 = ser.save(doc1, keepText = true)

    val doc2 = ser.load(out1)

    doc2.sentences.head.sections.value should be (firstSection)
    doc2.sentences.last.sections.value should be (lastSections)

    doc2.sentences.drop(1).dropRight(1) foreach { s =>
